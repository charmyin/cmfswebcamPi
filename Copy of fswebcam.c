...
int main(int argc, char *argv[])
{
	...
	else
	{
		...
		while(1 == 1)
		{
			...
			//generate the name
			countImage++;
			//char *newImgName = itoa(countImage, countImageStr, DECIMAL);
			snprintf(countImageStr, 20, "%d", countImage);
			printf("countImageStr = %s\n", countImageStr);
			strcat(countImageStr, "img.jpg");
			printf("countImageStr = %s\n", countImageStr);
			//config->filename
			//printf("OPT_SAVE=%d\n", OPT_SAVE);
			//printf("config->job[OPT_SAVE]->options = %s\n", config->job[47]->options);
			int xxx;
			for(xxx = 0; xxx < config->jobs; xxx++)
			{
				uint16_t id   = config->job[xxx]->id;
				char *options = config->job[xxx]->options;

				switch(id)
				{
					case OPT_SAVE:
							printf("++++++++++++config->job[x]->options %s\n", config->job[xxx]->options );
							//config->job[47]->options = malloc(sizeof(countImageStr));
							strcpy (config->job[xxx]->options, countImageStr);
							printf("++++++++++++config->job[x]->options %s\n", config->job[xxx]->options );
							break;
					default:
							break;
